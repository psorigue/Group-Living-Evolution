...
1. **01. Trait Reconstruction at Node**
    ...
    - [01.3. node_reconstruction_mbasr.R](Scripts/01.Trait%20Reconstruction%20at%20Node/01.3.node_reconstruction_mbasr.R) — Runs MBASR for Bayesian ancestral state reconstruction. This script estimates node-state probabilities using a Bayesian framework and produces the output files and plots for trait reconstruction.

2. **02. Read Mapping and Counting**
    - [02.1. STAR_genome_index_generation.sh](Scripts/02.Read%20Mapping%20and%20Counting/02.1.STAR_genome_index_generation.sh) — Builds the STAR genome index for the reference genome before read alignment.
    - [02.2. STAR_mapping.sh](Scripts/02.Read%20Mapping%20and%20Counting/02.2.STAR_mapping.sh) — Maps sequencing reads to the reference genome with STAR, generates BAM files, and indexes them for downstream analyses.
    - [02.3. count_table.sh](Scripts/02.Read%20Mapping%20and%20Counting/02.3.count_table.sh) — Uses featureCounts to count reads overlapping annotated features and generates a sample-by-gene count matrix.

3. **03. Differential Gene Expression**
    - [03.1. expression_PCA_and_outliers.R](Scripts/03.Differential%20Gene%20Expression/03.1.expression_PCA_and_outliers.R) — Performs PCA on normalized count data and detects potential outlier samples before differential expression analysis.
    - [03.2. differential_gene_expression.R](Scripts/03.Differential%20Gene%20Expression/03.2.differential_gene_expression.R) — Runs DESeq2 to identify differentially expressed genes between specified species/phenotype contrasts and writes the resulting DEG table with gene annotations.

4. **04. Functional Enrichment**
    - [04.1. GSEA_enrichment.R](Scripts/04.Functional%20Enrichment/04.1.GSEA_enrichment.R) — Performs gene set enrichment analysis for GO terms and KEGG pathways using ranked gene lists derived from DESeq2 results.
    - [04.2. intraspp_GO_similarity.R](Scripts/04.Functional%20Enrichment/04.2.intraspp_GO_similarity.R) — Computes semantic similarity among enriched GO terms within species-level comparisons, clusters related terms, and summarizes shared clusters.
    - [04.3. intraspp_KEGG_similarity.R](Scripts/04.Functional%20Enrichment/04.3.intraspp_KEGG_similarity.R) — Finds KEGG pathways shared between two intraspecies enrichment analyses and reports their descriptions and enrichment statistics.
    - [04.4. interspp_GO_similarity.R](Scripts/04.Functional%20Enrichment/04.4.interspp_GO_similarity.R) — Compares GO terms across interspecies contrasts, groups them by similarity, and highlights patterns shared between different comparisons.
