...
# Check all rows in metadata match a column in counts
...
# Variance-stabiolized transformation
vsd <- vst(dds_obj)

# plotPCA function only takes 500 more variable genes -> Use instead prcomp()
...
p <- ggplot(pca_data, aes(PC1, PC2,
  ...
  stat_ellipse(
    ...
    aes(group = interaction(species, phenotype),
        fill = species),   # ✅ IMPORTANT
    geom = "polygon",
    type = "t",
    level = 0.95,
    alpha = 0.3,
    color = NA             # optional: removes border
  ) +
...
results <- pca_data %>%
  ...
  do({
    ...
    # Flag outliers: e.g., distance > 3 SD
    threshold <- 3
    outlier_flag <- dist_z > threshold
...
# Save results
file_out <- paste0("//files1.igc.gulbenkian.pt/folders/ANB/Pol/Expression/group_vs_solitary/05.DEG/", 
                   region, "/outlier_zscore.txt")
