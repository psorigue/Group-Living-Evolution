...
#meta_fil <- meta[
#  meta$region == region & (
#    (meta$species == "Ornatipinnis" & meta$phenotype == "S") |
#      (meta$species == "Meeli" & meta$phenotype == "GL") |
#      (meta$species == "Multifasciatus" & meta$phenotype == "GL")
#  ), ]
meta_fil <- meta[meta$region == region & meta$species == "Meeli", ]
...
Sys.time()
OrS_TL = blockwiseModules(norm_cts,  # <= input here
...
# Save network
net_name <- "MeSMeGL_TL"
save(OrS_TL, file = paste0(path_wgcna, net_name, ".RData"))
Sys.time()
# Save datTraits
comp <- "MeSMeGL"
